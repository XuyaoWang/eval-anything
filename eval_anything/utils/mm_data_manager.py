    ...
    def prompt_to_conversation(
        ...
        if matches:
            ...
            for i, match in enumerate(matches):
                ...
                content_parts.append({"type": "image", "image": f"data:image/jpeg;base64,{cls.encode_image_to_base64(images[i])}"})
                last_end = match.end()
                
            if last_end < len(user_prompt):
                content_parts.append({"type": "text", "text": user_prompt[last_end:]})
        else:
            content_parts.extend([{"type": "image", "image": f"data:image/jpeg;base64,{cls.encode_image_to_base64(img)}"} for img in images])
            if user_prompt:
...
class AudioManager:
    ...
    def prompt_to_conversation(
        ...
        if len(audios) > 0 and not isinstance(audios[0], np.ndarray):
            loaded = [librosa.load(path, sr=sr if sr else None) for path, sr in zip(audios, sample_rates)]
        ...
        if matches:
            assert len(audios) == len(matches), f"Number of audios ({len(audios)}) does not match number of placeholders ({len(matches)}), input user_prompt: {user_prompt}"
            
            last_end = 0
            for i, match in enumerate(matches):
                if match.start() > last_end:
                    content_parts.append({"type": "text", "text": user_prompt[last_end:match.start()]})
                content_parts.append({"type": "audio_url", "audio_url": {"url": f"data:audio/wav;base64,{cls.encode_audio_to_base64(audios[i], sample_rates[i])}"}})
                last_end = match.end()
                
            if last_end < len(user_prompt):
                content_parts.append({"type": "text", "text": user_prompt[last_end:]})
        else:
            content_parts.extend([{"type": "audio_url", "audio_url": {"url": f"data:audio/wav;base64,{cls.encode_audio_to_base64(audio, sample_rate)}"}} for audio, sample_rate in zip(audios, sample_rates)])
            if user_prompt:
                content_parts.append({"type": "text", "text": user_prompt})
    ...
    @classmethod
    def extract_audios_from_conversation(cls, conversation: List[Dict]) -> List[Tuple[np.ndarray, int]]:
...
class VideoManager:
    ...
    # refer: https://github.com/QwenLM/Qwen2.5-VL/blob/main/README.md?plain=1#L817
    @classmethod
    def encode_video_to_base64(
        ...
        try:
            ...
            if isinstance(video, str):
                ...
                frames = [Image.fromarray(tensor[i].permute(1, 2, 0).numpy().astype(np.uint8)) 
                        for i in idx]
    ...
    @classmethod
    def extract_videos_from_conversation(cls, conversation: List[Dict]) -> Tuple[List[List[Image.Image]], Dict]:
        """
        ...
        for message in conversation:
            ...
            if isinstance(content, list):
                for item in content:
                    try:
                        if isinstance(item, dict) and 'type' in item and item['type'] == 'video_url' and 'video_url' in item and 'url' in item['video_url']:
    ...
    def prompt_to_conversation(
            ...
            videos: Union[str, List[Image.Image], np.ndarray, List[Union[str, List[Image.Image], np.ndarray]]] = [],
        ...
        if not isinstance(videos, list) or (len(videos) > 0 and 
                                           (isinstance(videos[0], Image.Image) or 
                                            (isinstance(videos[0], np.ndarray) and len(videos[0].shape) > 2))):
            videos = [videos]
        ...
        if matches:
            assert len(videos) == len(matches), f"Number of videos ({len(videos)}) does not match number of placeholders ({len(matches)}), input user_prompt: {user_prompt}"
            ...
            for i, match in enumerate(matches):
                ...
                content_parts.append({"type": "video_url", "video_url": {"url": f"data:video/jpeg;base64,{video_base64}"}, 'fps': fps_list[i]})
                ...
                # TODO: Remove this placeholder when the framwork support inference backend serving
